    ...
    if player_items and input("use an item? (y/anything else) ").lower() == "y":
        ...
        if "ball" in player_items[player_item_choice]:
            ...
            if len(valid_pokemons) > 0:
                ...
                heal_amount = {"potion": 25, "super potion": 50, "hyper potion": 150}[
                    player_items[player_item_choice]
                ]
            ...
            if (
                input("are you sure you want to use `book` (y/anything else)  ").lower()
                == "y"
            ):
                add_to_dict(player.items, player_items[player_item_choice], -1)
                try:
                ...
                if vel > 6.4:
                    print(
                        "you throw the book at the pokemon and it dies from internal bleeding"
                    )
                    sleep(1)
                else:
                    # print(victim)
                    gain = int(victim.exp * (random.random() + random.random() + 0.2))
                    print(
                        f"the book hits the pokemon at a perfect velocity and then the pokemon reads it and geains alot of life experience, giving it {gain} exp"
                    )
